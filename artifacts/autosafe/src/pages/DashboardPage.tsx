...
const isFresh = (s: Sensor) => Boolean(s.lastReadAt && Date.now() - new Date(s.lastReadAt).getTime() < 120_000);
const hasProblem = (s: Sensor) => {
  if (s.alertMuted) return false;
  if (!isFresh(s) && s.lastReadAt) return true;
...
export function DashboardPage() {
  ...
  const cards = useMemo(() => {
    ...
    if (filter === "offline") return base.filter((s) => !isFresh(s));
    return base;
  ...
  const handleMonitorAll = async () => {
    ...
    toast.info(`${MONITOR_LABEL[monitorMode]} — trzymaj aplikację otwartą i telefon w zasięgu czujników.`);
    const ok = await listenAll();
    if (!ok) {
      setMonitoring(false);
      toast.warning("Nie udało się uruchomić skanu BLE. Sprawdź Chrome, Bluetooth i uprawnienia.");
      return;
    }
        ...
        <section className="rounded-3xl border border-primary/25 bg-primary/5 p-4">
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Dodaj czujnik ELA, nadaj mu nazwę pomieszczenia, a potem użyj „Monitoruj wszystkie”. Dane zostają lokalnie w telefonie.</p>
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-hero p-7 text-white shadow-glow noise sm:p-8">
        ...
        <div className="relative z-10 flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm opacity-80">Temperatura, wilgotność, bateria, zasięg i problemy w jednym widoku.</p>
          </div>
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            ...
            {monitoring ? (
              ...
              <Button size="lg" onClick={handleMonitorAll} disabled={sensors.length === 0} className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-glow"><Radio className="mr-2 h-4 w-4" />Monitoruj wszystkie</Button>
            )}
        ...
        <div className="relative z-10 mt-6 grid grid-cols-2 gap-3 sm:grid-cols-5">
          ...
          <StatPill icon={<ShieldCheck className="h-4 w-4" />} label="Wymaga uwagi" value={`${problems.length}`} sub={problems.length ? "sprawdź niżej" : "wszystko OK"} />
        </div>
      </section>
        ...
        {(sensors.length > 0) && (
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">{connected.length} online · {sensors.length} zapisanych</p>
            </div>
