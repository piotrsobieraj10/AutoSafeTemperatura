...
import {
  addMeasurement, deleteSensor as storageDelete,
  getSettings, getSensors, upsertSensor,
...
export function useSensors() {
  ...
  // Advertisement watch dla czujników ELA/Ruuvi/Govee
  useEffect(() => {
    const advSensors = getSensors().filter(
      (s) => !s.isDemo && getProfile(s.profileId)?.source === "advertisement" && !watchedIds.current.has(s.id)
    );
    advSensors.forEach((sensor) => {
      const device = getCachedDevice(sensor.deviceId);
      if (!device) return;
      watchedIds.current.add(sensor.id);
      startAdvWatch(device, sensor.profileId, ({ data }) => {
        ...
        if (data.temperature !== undefined) {
          addMeasurement({ id: `${sensor.id}-${Date.now()}`, sensorId: sensor.id, roomName: sensor.roomName, temperature: data.temperature, humidity: data.humidity, pressure: data.pressure, rssi: data.rssi, batteryLevel: data.battery, createdAt: now });
        }
        setSensors(getSensors());
  ...
  const upsert = useCallback((s: Sensor) => { upsertSensor(s); refresh(); }, [refresh]);
